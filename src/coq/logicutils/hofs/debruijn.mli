 ...
 *)
  ...
  *)
 val unshift_local : env -> int -> int -> types -> types
 val shift_local : env -> int -> int -> types -> types
 
 (*
  * Shifting and unshifting all indices greater than 0 by an amount
  *)
 val unshift_by : env -> int -> types -> types
 val shift_by : env -> int -> types -> types
 
 (*
  * Shifting and unshifting all indices greater than 0
  *)
 val shift : env -> types -> types
 val unshift : env -> types -> types
 
 (* Shift everything and pray; workaround for bug *)
 val shift_by_unconditional : env -> int -> types -> types
  ...
  *)
 val shift_all : env -> types list -> types list
 val unshift_all : env -> types list -> types list
 
 (*
  * Shifting and unshifting a list by an amount
  *)
 val shift_all_by : env -> int -> types list -> types list
 val unshift_all_by : env -> int -> types list -> types list
  ...
  *)
 val shift_subs : env -> (types * types) list -> (types * types) list
 val shift_from : env -> (types * types) list -> (types * types) list
 val shift_to : env -> (types * types) list -> (types * types) list
