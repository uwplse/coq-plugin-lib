...
let py = init "."
let agent_utils = get_module py "agent_utils"
let prover = get_module py "prover"
...
let beam_search env prev goal = 
	...
	let ser_env = pp_format env in 
	let ser_goal = pp_format goal in 
	let filter_env = get_ref agent_utils "filter_env" [Pyref ser_env] in 
	let local_context, parsed_goal = get_ref agent_utils "parse_goal" [Pyref ser_goal] in 
	get_list prover "beam_search" [Pyref model; Pyref filter_env; Pyref local_context; Pyref parsed_goal; Pylist script] in